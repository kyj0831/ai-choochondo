...
| `APP_PASSWORD` | **강력 권장** | **운영자** 비밀번호. 사장님은 로그인 없이 자기 진단만 보고, 이 비밀번호로 로그인하면 전체가 보인다. 없으면 운영자 화면이 없고, 옛 진단(소유 키 없는 것)이 누구에게나 보인다 |
...
1. **비밀번호 게이트 확인.** 시크릿 창으로 접속해 로그인 화면이 뜨는지 직접 보세요.
   `APP_PASSWORD`를 안 넣으면 전체 목록을 볼 운영자가 없고, 옛 진단이 공개됩니다.
   (새로 만드는 진단은 만든 브라우저와 복구 링크로만 열립니다 — `lib/owner.ts`.)
